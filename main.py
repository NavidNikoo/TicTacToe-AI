        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if minimax_board[row][col] == 0:
                    ...
                    best_score = max(score, best_score) #If you want to beat minimax switch this with min
        return best_score
    else:
        best_score = 1000
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if minimax_board[row][col] == 0:
                    ...
                    minimax_board[row][col] = 0
                    best_score = min(score, best_score) #If you want to beat minimax switch this to max
        return best_score
